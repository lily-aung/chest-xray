...
def save_features_npz(out_dir, split, X, y, paths):
    ensure_dir(out_dir)
    np.savez_compressed(
...
def main():
    ...
    ap.add_argument("--img_size", type=int, default = 400)# default=224)
    ...
    ap.add_argument("--out_dir", default="artifacts/hog_highres_")
    args = ap.parse_args()
    ...
    Xte, yte, pte = build_split(args.test_csv, img_root, args.img_col, args.label_col,
                                ...
                                args.hog_orientations, args.hog_ppc, args.hog_cpb,
                                augment_train=False)
    save_features_npz(args.out_dir, "train", Xtr, ytr, ptr)
    #print("Saving feature")
    save_features_npz(args.out_dir, "val",   Xva, yva, pva)
    save_features_npz(args.out_dir, "test",  Xte, yte, pte)
    ...
    for name, clf in models.items():
        ...
        if name == "mlp":
            ...
            print("Original HOG dim:", Xtr.shape[1])
