...
def main():
    ...
    if mlflow_enabled:
        ...
        mlflow_run_id = init_mlflow(
            ...
            params={
                ...
                "custom_clahe": cfg.augmentation.custom_clahe,
    ...
    SAVE_DIR = getattr(cfg, "output", None)
    SAVE_DIR = getattr(SAVE_DIR, "dir", None) if SAVE_DIR is not None else None
    SAVE_DIR = SAVE_DIR or getattr(cfg, "out_dir", None) or "/content/drive/MyDrive/ChestXRay/checkpoints"

    os.makedirs(SAVE_DIR, exist_ok=True)

    save_path = os.path.join(SAVE_DIR, f"{run_name}_best_macro_f1.pt")

    state = trainer.best_state_dict_macro_f1
